class MessagesController < ApplicationController
  ...
  def create
    ...
    @message.resourcable = current_user
    if @message.save
      if params[:document_ids].present?
        params[:document_ids].split(',').each do |document_id|
          @message.document_documentables.create(document_id: document_id, resourcable: current_user)
          @object.document_documentables.find_or_create_by(document_id: document_id, resourcable: current_user)  if @object.class.name != "Ticket" && @object.class.name != "Project"
        end
      end
    ...
    respond_to do |format|
      ...
      if params[:commit] == "Post Comment"
        format.html {redirect_to @object.class.name == 'Project' ? project_message_path(@object, @message.messageable) : ticket_message_path(@object, @message.messageable) }
      else
        format.html {redirect_to @object.class.name == 'Project' ? project_path(@object) : ticket_path(@object)}
      end
    end
  end
  ...
  def show
    ...
    @project_contacts = ProjectContact.where(project_id: params[:project_id])
    @users = User.where(id: [@project_users.pluck(:user_id)])
    @contacts = Contact.where(id: @project_contacts.pluck(:contact_id))
  ...
  def edit
    ...
    @users = current_user.all_user_of_related_company.active.active
    @klass = Klass.find_by(name: "Message")
    @groups = Group.includes(:fields).where(klass: @klass)
    user_details = @users.pluck(:first_name, :id)
  ...
  def update
    ...
    @message_fields = current_user.fields_for_table_with_order(klass: @message_klass)
    if params[:project_id].present?
      @object = Project.find(params[:project_id])
    elsif params[:ticket_id].present?
      @object = Ticket.find(params[:ticket_id])
    ...
    if @message.update(message_params)
      if params[:document_ids].present?
        ...
        params[:document_ids].split(',').each do |document_id|
          @message.document_documentables.find_or_create_by(document_id: document_id, resourcable: current_user)
          @object.document_documentables.find_or_create_by(document_id: document_id, resourcable: current_user) if @object.class.name != "Ticket" && @object.class.name != "Project"
        end
  ...
  def reply_update
    ...
    if params[:document_ids].present?
      params[:document_ids].split(',').each do |document_id|
        @message.document_documentables.create(document_id: document_id, resourcable: current_user)
        #@ticket.document_documentables.create(document_id: document_id)
      end
    end
    @klass = Klass.find_by(name: "Ticket")
    @overviews = (@ticket.messages  + @ticket.document_documentables + @ticket.messages.each{ |doc| doc.documents } ).sort{|a,b| b.created_at <=> a.created_at }.group_by{|t| t.created_at.to_date }
  ...
  private
   def message_params
    @klass = Klass.find_by(name: "Message")
    params.require(:message).permit(@klass.fields.pluck(:name), :messageable_type, :messageable_id, :resourcable_type , :resourcable_id)
  end
end
