class Contacts::DirectoriesController < ApplicationController
  layout 'contacts'
  ...
  def create
    @directory = Directory.new(directory_params.merge({ resourcable: current_contact}))
    if @directory.save
  ...
  def system_files
    @directory = Directory.where(directoriable: current_contact.lead_client_contact.contactable).where(name: 'system_dir').first
    @documents = @directory.documents
  end
