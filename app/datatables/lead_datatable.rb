class LeadDatatable < AjaxDatatablesRails::ActiveRecord
  ...
  def view_columns
    @view_columns ||= begin
                        ...
                        fields.each do |field|
                          columns[field.name] = { source: "Lead.#{field.name}" }
                        end
                        # For date and datetime filtering
                        fields.select{|f| f.is_date_or_date_time? }.each do |field|
                          columns[field.name] = { source: "Lead.#{field.name}", cond: :date_range, delimiter: '-yadcf_delim-' }
                        end
